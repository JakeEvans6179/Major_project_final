...
from tensorflow.keras.layers import Input, LSTM, Dense, Dropout, Conv1D, SimpleRNN
...
'''
Simple RNN centralised validation screening on final cohort
'''
...
plt.title('Centralized RNN: Training vs Validation Loss')
plt.xlabel('Epochs')
...
#save plot
plt.savefig(plots_dir / "centralised_RNN_Dense_learning_curve.png", dpi=200)
plt.close()
...
results_df.to_csv("centralised_RNN_per_house_validation_eval.csv", index=False)
...
summary_df.to_csv("centralised_SimpleRNN_validation_summary.csv", index=False)
print(summary_df)

model.save("RNN_Dense_global.keras")
