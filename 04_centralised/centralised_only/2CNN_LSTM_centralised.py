...
'''
2CNN-LSTM centralised validation screening on final cohort
'''

plots_dir = Path("train_val_curve")
plots_dir.mkdir(exist_ok=True) #creates directory if it doesn't exist
...
#plot learning curve for train and validation loss
plt.figure(figsize=(10, 6))
plt.plot(history.history['loss'], label='Training Loss (MSE)')
plt.plot(history.history['val_loss'], label='Validation Loss (MSE)')

#show best epoch as vertical line
...
plt.title('Centralized 2CNN-LSTM: Training vs Validation Loss')
plt.xlabel('Epochs')
...
plt.savefig(plots_dir / "centralised_2CNN_LSTM_learning_curve.png", dpi=200)
plt.close()
...
results_df.to_csv("centralised_2CNN_LSTM_per_house_validation_eval.csv", index=False)
...
summary_df.to_csv("centralised_2CNN_LSTM_validation_summary.csv", index=False)
print(summary_df)

model.save("2CNN_LSTM_global.keras")
