...
'''
CNN-lstm-dense centralised validation screening on final cohort --test no dropout after CNN layer
'''
...
plt.title('Centralized CNN-LSTM-Dense --test no dropout--: Training vs Validation Loss')
plt.xlabel('Epochs')
...
plt.savefig(plots_dir / "test_learning_curve.png", dpi=200)
plt.close()
...
results_df.to_csv("test_per_house.csv", index=False)
