...
'''
CNN-2LSTM-Dense centralised validation screening on final cohort
'''
...
plt.title('Centralized CNN-2LSTM-Dense: Training vs Validation Loss')
plt.xlabel('Epochs')
...
plt.savefig(plots_dir / "centralised_CNN_2LSTM_Dense_learning_curve.png", dpi=200)
plt.close()
...
results_df.to_csv("centralised_CNN_2LSTM_Dense_per_house_validation_eval.csv", index=False)
...
summary_df = pd.DataFrame([{
    "model": "centralised_CNN_2LSTM_Dense",
...
summary_df.to_csv("centralised_CNN_2LSTM_Dense_validation_summary.csv", index=False)
print(summary_df)

model.save("CNN_2LSTM_Dense_global.keras")
