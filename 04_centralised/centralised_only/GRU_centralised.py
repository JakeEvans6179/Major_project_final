...
from tensorflow.keras.layers import Input, LSTM, Dense, Dropout, Conv1D, SimpleRNN, GRU
...
'''
GRU centralised validation screening on final cohort
'''
...
plt.title('Centralized GRU: Training vs Validation Loss')
plt.xlabel('Epochs')
...
#save plot
plt.savefig(plots_dir / "centralised_GRU_learning_curve.png", dpi=200)
plt.close()
...
results_df.to_csv("centralised_GRU_per_house_validation_eval.csv", index=False)
...
summary_df.to_csv("centralised_GRU_validation_summary.csv", index=False)
print(summary_df)

model.save("GRU_global.keras")
