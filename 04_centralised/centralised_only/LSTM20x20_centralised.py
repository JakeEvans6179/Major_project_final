...
'''
LSTM20x20 centralised validation screening on final cohort
'''
...
plt.title('Centralized LSTM20x20: Training vs Validation Loss')
plt.xlabel('Epochs')
...
plt.savefig(plots_dir / "centralised_LSTM20x20_learning_curve.png", dpi=200)
plt.close()
...
results_df.to_csv("centralised_LSTM20x20_per_house_validation_eval.csv", index=False)
...
summary_df.to_csv("centralised_LSTM20x20_validation_summary.csv", index=False)
print(summary_df)

model.save("LSTM20x20_global.keras")
