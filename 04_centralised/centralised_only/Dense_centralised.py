...
'''
Dense centralised validation screening on final cohort
'''
...
plt.title('Centralized Dense: Training vs Validation Loss')
plt.xlabel('Epochs')
...
plt.savefig(plots_dir / "centralised_Dense_learning_curve.png", dpi=200)
plt.close()
...
results_df.to_csv("centralised_Dense_per_house_validation_eval.csv", index=False)
...
summary_df.to_csv("centralised_Dense_validation_summary.csv", index=False)
print(summary_df)

model.save("Dense_global.keras")
