...
'''
Literature S2S autoencoder structure centralised validation screening on final cohort

LSTM50 encoder, latent space, LSTM50 decoder, TimeDistributed Dense(100) + TimeDistributed Dense(1) output head
'''
...
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import Input, LSTM, Dense, RepeatVector, TimeDistributed, Flatten
from tensorflow.keras.optimizers import Adam
import tensorflow as tf
...
plt.title('Centralized S2S: Training vs Validation Loss')
plt.xlabel('Epochs')
...
results_df.to_csv("centralised_S2S_per_house_validation_eval.csv", index=False)
...
summary_df.to_csv("centralised_S2S_validation_summary.csv", index=False)
print(summary_df)

model.save("S2S_global.keras")
