...
'''
LSTM64x32 centralised validation screening on final cohort
'''
...
plt.title('Centralized LSTM64x32: Training vs Validation Loss')
plt.xlabel('Epochs')
...
plt.savefig(plots_dir / "centralised_LSTM64x32_learning_curve.png", dpi=200)
plt.close()
...
results_df.to_csv("centralised_LSTM64x32_per_house_validation_eval.csv", index=False)
...
summary_df.to_csv("centralised_LSTM64x32_validation_summary.csv", index=False)
print(summary_df)

model.save("LSTM64x32_global.keras")
