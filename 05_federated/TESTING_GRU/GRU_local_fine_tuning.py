...
'''
GRU validation metrics (Federated fine tuning)
...
global_model_path = Path("chunk_checkpoints/global_chunk_038_GRU.keras")  #find the best model from validation screening and use for fine tuning
...
results_df.to_csv("fine_tuned_GRU_per_house.csv", index=False)
...
summary_df.to_csv("fine_tuned_GRU_summary.csv", index=False)
print(summary_df)
