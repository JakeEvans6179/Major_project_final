...
then apply row-wise z-score normalisation so clustering focuses on SHAPE rather
...
2. household_24h_train_profiles_rowz.csv
   - same layout, but each row z-scored across its own 24 hours

3. kmeans_k_sweep_metrics_rowz.csv
   - clustering metrics for each k

4. kmeans_assignments_rowz_k{k}.csv
   - house_id to cluster assignment for each k

5. cluster_centroids_rowz_k{k}.png
...
OUT_ROWZ = Path("household_24h_train_profiles_rowz.csv")
OUT_METRICS = Path("kmeans_k_sweep_metrics_rowz.csv")
...
for k in K_VALUES:
    ...
    assignments.to_csv(f"kmeans_assignments_rowz_k{k}.csv", index=False)
...
for k in K_VALUES:
    ...
    plt.title(f"Average 24-Hour Profiles for k={k} (clustered on row-z features)")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"cluster_centroids_rowz_k{k}.png", dpi=150)
    plt.close()
