...
'''
Testing for CNN_LSTM
6 hour forecasting horizon 

validation metrics
'''

plots_dir = Path("house_plots_cnn_lstm")
...
for i, house_id in enumerate(house_ids, start=1):
    ...
    plt.ylabel("kWh")
    plt.title(f"{house_id} | 6-step forecast | CNN_LSTM | Horizon t+1")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("Val sample index")
    plt.ylabel("kWh")
    plt.title(f"{house_id} | 6-step forecast | CNN_LSTM | Horizon t+3")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("Val sample index")
    plt.ylabel("kWh")
    plt.title(f"{house_id} | 6-step forecast | CNN_LSTM | Horizon t+6")
    plt.legend()
    plt.tight_layout()
...
results_df.to_csv("cnn_lstm_localised_per_house_results.csv", index=False)
...
summary_df.to_csv("cnn_lstm_localised_summary.csv", index=False)
print(summary_df)
