...
from tensorflow.keras.layers import Input, LSTM, Dense, Dropout, Conv1D, GRU
...
plots_dir = Path("house_plots_gru64x32")
...
for i, house_id in enumerate(house_ids, start=1):
    ...
    plt.ylabel("kWh")
    plt.title(f"{house_id} | 6-step forecast | GRU | Horizon t+1")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("Val sample index")
    plt.ylabel("kWh")
    plt.title(f"{house_id} | 6-step forecast | GRU | Horizon t+3")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("Val sample index")
    plt.ylabel("kWh")
    plt.title(f"{house_id} | 6-step forecast | GRU | Horizon t+6")
    plt.legend()
    plt.tight_layout()
...
results_df.to_csv("gru64x32_localised_per_house_results.csv", index=False)
...
summary_df.to_csv("gru64x32_localised_summary.csv", index=False)
print(summary_df)
