...
partial_results_path = Path("SARIMA_per_house_results_6hr_partial.csv")
partial_search_path = Path("SARIMA_candidate_search_results_6hr_partial.csv")

if partial_results_path.exists():
    results_df_existing = pd.read_csv(partial_results_path)
    results_df_existing = results_df_existing.drop_duplicates(subset=["house_id"], keep="last")
    done_house_ids = set(results_df_existing["house_id"].astype(str).tolist())
...
if partial_search_path.exists():
    search_results_existing = pd.read_csv(partial_search_path)
    all_search_dfs = [search_results_existing]
else:
    all_search_dfs = []

house_ids = [h for h in house_ids if h not in done_house_ids]

print(f"Houses remaining to run: {len(house_ids)}")
