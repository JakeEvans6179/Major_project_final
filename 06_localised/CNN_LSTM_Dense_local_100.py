...
'''
Testing for CNN_LSTM_Dense
6 hour forecasting horizon 

validation metrics
'''

plots_dir = Path("house_plots_cnn_lstm_dense")
...
for i, house_id in enumerate(house_ids, start=1):
    ...
    plt.ylabel("kWh")
    plt.title(f"{house_id} | 6-step forecast | CNN_LSTM_Dense | Horizon t+1")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("Val sample index")
    plt.ylabel("kWh")
    plt.title(f"{house_id} | 6-step forecast | CNN_LSTM_Dense | Horizon t+3")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("Val sample index")
    plt.ylabel("kWh")
    plt.title(f"{house_id} | 6-step forecast | CNN_LSTM_Dense | Horizon t+6")
    plt.legend()
    plt.tight_layout()
...
print("\nPer-house CNN_LSTM_Dense results:")
print(results_df.head())

print("\nCNN_LSTM_Dense summary:")
...
results_df.to_csv("cnn_lstm_dense_localised_per_house_results.csv", index=False)


#summary statistics
summary_df = pd.DataFrame([{
    "model": "cnn_lstm_dense",
...
summary_df.to_csv("cnn_lstm_dense_localised_summary.csv", index=False)
print(summary_df)
